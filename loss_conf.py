...
class ConfidenceLoss(nn.Module):
    def __init__(self, main_model: nn.Module, confidence_model, config, eps=1e-3):
        super().__init__()
    ...
    def forward(self, input_batch) -> Tensor:
        ...
        with torch.no_grad():
            sample = self.main_model.decode_euler(z, batch)[-1]
        ...
        edge_dist = torch.linalg.norm(sample[:,None] - sample[:,:,None],dim=-1).unsqueeze(-1)
        adj_onehot = F.one_hot(adj.long(), num_classes=5)
        edge_feat = torch.cat([adj_onehot, edge_dist],dim=-1) * pair_mask[...,None]
        x = self.confidence_model(node_feat, sample, edge_feat, mask)
        loss = lddt_loss(x, sample, coords, mask, batch['ncaa_mask'])
