...
from dataset_csd import get_dataloader as get_dataloader_csd
from dataset_pubchem import get_dataloader as get_dataloader_pubchem
...
class Trainer3D(object):
    def __init__(self, config, ddp=False, device=None):
        ...
        if self.config.data.data == 'csd':
            self.train_loader, self.test_loader, self.train_sampler, self.test_sampler = get_dataloader_csd(self.config,ddp=ddp)
        elif self.config.data.data == 'pubchem':
            self.train_loader, self.test_loader, self.train_sampler, self.test_sampler = get_dataloader_pubchem(self.config,
                                                                                                            ddp=ddp)
        else:
